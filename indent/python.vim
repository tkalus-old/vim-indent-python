    ...
    if parlnum > 0
        ...
        if match(getline(parlnum), '[([{]\s*$', parcol - 1) != -1
            ...
            if closing_paren
                ...
                " Respect the number of leading spaces after the paren
                " This makes vim indent like this:
                " def foo( bar,
                "          qux
                let results = matchlist(getline(parlnum), '[([{]\(\s*\).\{-}$', parcol - 1)
                let num_spaces = strlen(results[1])
                return parcol + num_spaces
            endif
        endif
    endif
